...
var templogreport;
var filenumber=0;
var configdict = { mapselected: null,
                carselected: null,
...
function clearlogbutton(){
    ...
    container[0].style.background ="linear-gradient(to bottom, #00ff57 0%, #016322 100%)";
    setInterval(function(){container[0].style.background = "linear-gradient(to bottom, #4a4d52 0%, #212327 100%)";},1000)
    checkboxelement.checked = false;
}
...
function existbutton(){
    ...
    var today = new Date();
    templogreport = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds() + ": " + text;
    var container = document.getElementsByClassName("existbutton");
    var text = document.getElementById("existtext");
    var checkboxelement = document.getElementById("existcheckbox");
    checkboxelement.checked = true;
    Logrecord("Log Exported");
    container[0].style.background ="linear-gradient(to bottom, #00ff57 0%, #016322 100%)";
    setInterval(function(){container[0].style.background = "linear-gradient(to bottom, #4a4d52 0%, #212327 100%)";},1000)
    checkboxelement.checked = false;
    filenumber+=1;
    filename = today.getFullYear()+":"+ month[today.getUTCMonth()]+":" + today.getUTCDate()+ "_Log_Record "+ filenumber.toString();
    eel.exportfile(filename,loglist);
}
